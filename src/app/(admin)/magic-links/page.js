...
export default function MagicLinksAdminPage() {
    const [tenant, setTenant] = useState("NF");
    ...
    // Auto-update redirect URL whenever tenant changes
    useEffect(() => {
        if (tenant && TENANT_DOMAINS[tenant]) {
            setRedirectUrl(`${TENANT_DOMAINS[tenant]}/assigned-articles`);
            fetchLinks(); 
        }
    }, [tenant]);

    // Fetch existing magic links
    async function fetchLinks() {
        setFetching(true);
        const res = await fetch(`/api/magic-link/list?tenant=${tenant || "NF"}`);
    ...
    async function generateLink() {
        ...
        await fetch("/api/magic-link/create", {
            ...
            body: JSON.stringify({ tenant, email, redirectUrl }),
        });
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {/* Redirect (auto-generated & disabled) */}
                    <div>
            ...
            {/* Magic Links Table */}
            <div className="bg-white p-6 rounded-xl shadow border">
                <h2 className="text-xl font-semibold mb-4">Existing Links</h2>
                                ...
                                <tr key={link.id} className="border-b">
                                    ...
                                    <td className="p-2 space-x-2">
                                        ...
                                        <button
                                            ...
                                            onClick={() =>
                                                deleteLink(link.id, link.tenant)
                                            }
                                        >
                                            Delete
                                        </button>
